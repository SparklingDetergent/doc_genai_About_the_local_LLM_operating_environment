...
| | NPU | AI処理に特化 | 非常に高 | 中規模～大規模モデル | 最新モデルに対応 | 対応機種が限られる |
| **メモリ** | RAM | システムメモリ、高速アクセス | 中 | モデルと中間結果 | 容量不足でも拡張しやすい |
| | VRAM | GPU搭載メモリ、高速アクセス | 高（GPU使用時） | モデルと中間結果 | 容量不足に注意 |
| | ユニファイドメモリ | CPU/GPU共有 | 高（適切な設計時） | 最新モデル | 省電力。容量不足に注意。対応機種が限られる |
...
| | gguf | 圧縮形式 | 高 | 中規模モデル | 推論速度と圧縮率のバランス | llama.cpp 向け |
...
- **NPU**: ニューラルネットワーク処理に特化したチップで、GPUよりも高速かつ省電力です。中規模から大規模モデルの処理に適しており、最新モデルへの対応も期待できます。対応機種が限られます。
...
- **ユニファイドメモリ**: CPUとGPUが共有できるメモリで、両方の利点を活かせます。拡張性が低いので、導入時に容量不足だと利用したいモデルのサイズによっては実行不可となります。対応機種が限られます。
...
- **gguf**: llama.cpp 向けに開発された圧縮形式で、中規模モデルの保存に適しています。推論速度と圧縮率のバランスが取れています。
